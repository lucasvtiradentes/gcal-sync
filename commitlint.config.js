      ...
      rules: {
        "gitmoji-workflow": (parsed) => {
          ...
          if (emoji === null) {
            return [
              false,
              `header must specify an emoji like -> ${VALID_COMMIT_MESSAGE}`,
            ];
          }
          if (type === null) {
            return [
              false,
              `header must specify an type like -> ${VALID_COMMIT_MESSAGE}`,
            ];
          }
          if (VALID_EMOJI_TYPES[type] !== emoji) {
            return [
              false,
              `the speficied emoji does not match its type as it should -> ${VALID_EMOJI_TYPES[type]} !== ${emoji}`,
            ];
          }
          if (subject === null) {
            return [
              false,
              `header must specify an subject like -> ${VALID_COMMIT_MESSAGE}`,
            ];
          }
